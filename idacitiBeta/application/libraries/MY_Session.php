...
class MY_Session
{
	...
	{
		log_message('info', "MY_Session Class Initialized");
		...
		if (count($newdata) > 0)
		{
			foreach ($newdata as $key => $val)
			{
				if (array_key_exists($key, $_SESSION)) {
					log_message('debug', 'session.set_userdata called with '.$key.' already set: ' . print_r($_SESSION[$key], TRUE));
					log_message('debug', 'session.set_userdata new value: ' . print_r($val, TRUE));

				}
				else {
					log_message('debug', 'session.set_userdata called with '.$key.' NOT set');
